...
//                .avatar(user.getAvatar()) // Added
//                .bio(user.getBio())       // Added
//                .address(user.getAddress()) // Added
//                .city(user.getCity())       // Added
//                .state(user.getState())     // Added
//                .zipCode(user.getZipCode()) // Added
//                .dateOfBirth(user.getDateOfBirth()) // Added
...
public class AuthService {
    ...
    private final JwtService jwtService;
    private final AuthenticationManager authenticationManager;
    private final EmailService emailService; // Brevo Integration
    private final RestTemplate restTemplate = new RestTemplate();
    ...
    public void verifyOtp(VerifyOTPRequest request) {
        ...
        Otp otp = otpRepository.findByEmailAndCode(cleanEmail, request.getOtp())
                .orElseThrow(() -> new BusinessException("Invalid OTP code or email."));

        if (otp.getExpiresAt().isBefore(LocalDateTime.now())) {
            otpRepository.delete(otp); // Clean up expired code
            throw new BusinessException("OTP has expired. Please request a new one.");
        }

        otpRepository.delete(otp); // Success: Delete so it can't be reused
    }

    // --- 7. KINGSCHAT INTEGRATION ---
    @Transactional
    public AuthResponse signInWithKingsChat(String kingschatAccessToken) {
        try {
            KingsChatProfile profile = verifyKingsChatToken(kingschatAccessToken);
            ...
            User user = userRepository.findByKingschatId(kcUserId)
                    .orElse(userRepository.findByEmail(kingschatEmail).orElse(null));
    ...
    private KingsChatProfile verifyKingsChatToken(String accessToken) {
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                JsonNode root = objectMapper.readTree(response.getBody());
                JsonNode profileNode = root.get("profile");
                return profileNode == null ? objectMapper.treeToValue(root, KingsChatProfile.class) : objectMapper.treeToValue(profileNode, KingsChatProfile.class);
            }
            return null;
        } catch (Exception e) {
            throw new BusinessException("Failed to verify KingsChat token");
        }
    }
